...
class Vector2f {
    static constexpr double fuzzyEpsilon = 0.0001;

public:
    GLfloat x, y;

    bool fuzzyEquals(const Vector2f &other) const {
        if (this->x - other.x >= fuzzyEpsilon) return false;
        if (this->y - other.y >= fuzzyEpsilon) return false;
        return true;
    }
};
