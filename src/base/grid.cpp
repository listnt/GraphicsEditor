...
grid::grid(float step) : step(step) {
    ...
    for (float x = -100; x < 100; x += step) {
        for (float y = -100; y < 100; y += step) {
    ...
    colors = std::vector<Vector4f>(points.size(), {0.3, 0.3, 0.3, 1});
    pickingColor = std::vector<Vector4f>(points.size(), {0, 0, 0, 0});
